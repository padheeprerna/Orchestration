...
# copy CSS to the destination report path
...
def generateSASTReport(inputfile, reportPath, flag):
    out_new_path = reportPath + "//SAST-Analysis-Report-" + myFileName + ".html"
    ...
    htmlfile.write(
        ...
        'class="art-logo-name">SAST Security Assessment Report</h1></div>')
    ...
    htmlfile.write(
        '<p style="font-weight:bold;"><left><b><h1 style="color:blue;"> SAST Tools Assessment Results '
        '</h1></b></left></p>')

    # START - Write results of OWASP Dependency Check scan
    htmlfile.write(
        '<br><p style="font-weight:bold;color:blue;font-size:20px"><left><b>i) OWASP Dependency Check Results '
        '</b></left></p><br>')
    dataFrame = pd.read_csv(inputfile, usecols=[3, 10, 11, 12, 14])
    if dataFrame.empty:
        htmlfile.write(
            '<br><p style="font-size:18px"><left><b> No Components/Dependencies with Known Vulnerabilities found! '
            '</left></p>')
    else:
